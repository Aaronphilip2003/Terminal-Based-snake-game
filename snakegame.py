...
win.nodelay(1) # we are not waiting for the user to hit the next key, so we have set nodelay to true ie 1
...
while key!=ESC:
    ...
    win.timeout(150-(len(snake))//5 + len(snake)//10 %120) # Basic algorithm to increase the speed of the snake based on its length
